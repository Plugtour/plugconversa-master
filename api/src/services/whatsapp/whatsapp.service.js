...
async function startSession(tenantId, options = {}) {
  ...
  sock.ev.on("messages.upsert", async (payload) => {
    ...
    try {
      ...
      // ✅ detecta se a conversa já existia (para emitir conversation_new)
      const convExistsResult = await client.query(
        `
        SELECT id
          FROM conversations
         WHERE tenant_id = $1
      ...
      const conversationAlreadyExists = convExistsResult.rowCount > 0;
      ...
      // 3) message (direction='in') + RETURNING (pra emitir via SSE)
      ...
      // 4) atualiza preview da conversa (mostra na lista do Inbox)
      await client.query(
        `
      ...
      // ✅ emite SSE (conversation_new quando for nova)
      try {
        if (!conversationAlreadyExists) {
          sseBroadcast(t, "conversation_new", {
            tenant_id: t,
        ...
        // ✅ sempre avisa que a conversa mudou (lista do Inbox)
        sseBroadcast(t, "conversation_update", {
          tenant_id: t,
          conversation_id: conversationId,
        });
      } catch (e) {
        logErr(t, "SSE_CONVERSATION_BROADCAST_ERROR", e?.message || e);
      }
